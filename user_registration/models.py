...
import logging
logger = logger = logging.getLogger('filelogger')
...
class RegistrationProfile(models.Model):
    ...
    def send_activation_email(self, site, request=None):
        ...
        message_txt = render_to_string(activation_email_body,
                                       ctx_dict)
        # email_html = render_to_string(activation_email_html,
        #                                 ctx_dict)
        ...
        html_content = render_to_string(settings.ACTIVATION_EMAIL_HTML, ctx_dict)
        email = EmailMessage(
            "Активация аккаунта mailerfeed", 
            html_content, 
            settings.DEFAULT_FROM_EMAIL,
            [self.user.email,])
